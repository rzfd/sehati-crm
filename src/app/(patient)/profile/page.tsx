...
export default function PatientProfilePage() {
  ...
  if (loading) return <div className="p-6 lg:p-8 text-body-md text-ink-muted">Memuat…</div>
  if (!patient) return <div className="p-6 lg:p-8 text-body-md text-ink-muted">Sesi tidak ditemukan.</div>

  const initials = (patient.name ?? "?").split(" ").map((p) => p[0]).slice(0, 2).join("").toUpperCase()

  return (
    <div className="p-6 lg:p-8 max-w-5xl">
      <h1 className="text-headline-lg text-ink mb-5">Profil Saya</h1>
